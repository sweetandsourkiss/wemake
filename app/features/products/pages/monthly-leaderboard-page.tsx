...
export function meta({ data }: Route.MetaFunction) {
  const { year, monthName } = data;
  return [
    { title: `${monthName} ${year} Product Leaderboard - WeMake` },
    {
      name: "description",
      content: `Top products on WeMake in ${monthName} ${year}`,
    },
  ];
}
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-8">
        {monthName} {year} Product Leaderboard
      </h1>
      ...
      <div className="mb-8">
        <div className="flex flex-wrap gap-2">
          {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
            <a
              key={m}
              href={`/products/leaderboards/monthly/${year}/${m}`}
              className={`px-4 py-2 rounded-md ${
                m === month
                  ? "bg-primary text-primary-foreground"
                  : "bg-muted hover:bg-muted/80"
      ...
      {products.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground text-lg">
            No products found for {monthName} {year}.
          </p>
        </div>
      )}
    </div>
